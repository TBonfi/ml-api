...
dashboard.bind(app)
@app.route('/predict/saved_models/v1/<model>', methods=['POST'])
def predict(model):
...
def hello_not_really_stranger(nombre):
    return f"Hola {nombre}! viste que tremenda API? (by the way usaste el método {request.method})"

@app.route('/predict/saved_preds/v01/<model>', methods=['POST'])
def retrieve(model):
